...
class GeoLens(DeepObj):
    ...
    def sample_parallel(self,fov=0.0, R=None, z=None, M=15,  wvln=DEFAULT_WAVE, sampling='grid', forward=True, entrance_pupil=False):
        """ Sample parallel rays from plane (-R:R, -R:R, z). Rays have shape [spp, M, M, 3]
            ...
            FIXME: I totally forget the purpose to design this function. Should re-write this function in the future.
    ...
    def sample_point_source(self, R=None, depth=-10.0, M=11, spp=GEO_SPP, fov=10.0, forward=True, pupil=True, wvln=DEFAULT_WAVE, importance_sampling=False):
        ...
        # =========>
        # o = self.point_source_grid(depth=depth, grid=M, normalized=True, center=False)
        # o = o.unsqueeze(0).repeat(spp, 1, 1, 1)
        # =========>
        ...
        if pupil:
            ...
            # phi = (torch.rand(spp, M, M)*2*math.pi).to(self.device)   # [0, 2pi]
            # if forward:
            #     d = torch.stack((
    ...
    def sample_sensor(self, spp=64, pupil=True, wvln=DEFAULT_WAVE, sub_pixel=False):
        ...
        if sub_pixel:
            ...
            # Sub-pixel sampling, used in realistic rendering
            delta_x = torch.rand((spp, self.sensor_res[0], self.sensor_res[1]), device=self.device) * self.pixel_size
            delta_y = torch.rand((spp, self.sensor_res[0], self.sensor_res[1]), device=self.device) * self.pixel_size
            x1 = x1 - delta_x
            y1 = y1 + delta_y
            ...
            # weight = []
            # for _ in range(spp):    
            #     # regarding a pixel as a square box. x_jitter should be in [-1, 0], y_jitter should be in [0, 1]
            #     x1 = x1 + (torch.rand(*x1.shape, device=self.device) - 1) * self.pixel_size
            #     y1 = y1 + torch.rand(*y1.shape, device=self.device) * self.pixel_size
            #     o1 = torch.stack((x1, y1, z1), 2)

            #     theta = torch.rand((vpp,self.sensor_res[0],self.sensor_res[1]), device=self.device) * 2 * math.pi
            #     r = torch.sqrt(torch.rand((vpp,self.sensor_res[0],self.sensor_res[1]), device=self.device) * pupilr**2)
            #     x2 = r * torch.cos(theta)
            #     y2 = r * torch.sin(theta)
            #     z2 = torch.full_like(x2, pupilz, device=self.device)
            #     o2 = torch.stack((x2, y2, z2), -1)
            #     weight_ = nnF.normalize(x2**2 + y2**2 + pupilr**2, p=2, dim=0)
            #     weight_ = weight_ / torch.sum(weight_, 0)  # normalize to sum 1

            #     o1 = torch.broadcast_to(o1, o2.shape)
            #     d_ = o2 - o1    # broadcast to [vpp, H, W, 3]
            #     o.append(o1)
            #     d.append(d_)
    ...
    def trace(self, ray, lens_range=None, record=False):
        """ General ray tracing function. Ray in and ray out.
            ...
            FIXME: the definition of oss in record = True is stupid...
    ...
    def trace2sensor(self, ray, record=False, ignore_invalid=False):
        """ Trace optical rays to sensor plane.
            ...
            # FIXME: the original design of this case is too stupid to modify...
        """
        if record:
    ...
    def forward_tracing(self, ray, lens_range, record):
        ...
        for i in lens_range:
            ...
            if record: 
                p = ray.o
                for os, v, pp in zip(oss, valid.cpu().detach().numpy(), p.cpu().detach().numpy()):
                    if v.any(): # valid, FIXME: not secure here, because not every ray is valid
                        os.append(pp)
    ...
    def render(self, img, depth=DEPTH, spp=64, psf_grid=9, psf_ks=101, noise=0.0, method='ray_tracing'):
        """ This function is defined for End-to-End lens design. 
            ...
            TODO: I am planning to support 2 kinds of rendering methods:
                1. ray tracing based rendering
                2. PSF based rendering

            TODO: We should also implement non-differentiable, but more accurate rendering.
                1. high spp
    ...
    def render_compute_image(self, img, depth, scale, ray):
        """ Ray tracing rendering step2: ray and texture plane intersection and computer rendered image.
            ...
            # TODO: only receive [N, C, H, W] or [N, H, W] tensor in the future
    ...
    def render_high_res(self, img, depth, scale, patch_len=16, spp=4096, method='stack'):
        """ Split the full-resolution image into patches and render each patch separately.

            TODO: finish this function.

            method: 'patch' or 'stack'
        """
        # B, C, H, W = img.shape
        B, C, _, _ = img.shape
        H, W = self.sensor_res
        img_out = torch.zeros((B, C, H, W), device=self.device) + 1j * torch.zeros((B, C, H, W), device=self.device)
        ray = self.sample_sensor(spp=spp, wvln=DEFAULT_WAVE)

        for pi in range(int(np.ceil(H/patch_len))):    # int function here is not accurate
            for pj in range(int(np.ceil(W/patch_len))):
                low_i = pi * patch_len
                up_i = min((pi+1)*patch_len, H)
                low_j = pj * patch_len
                up_j =  min((pj+1)*patch_len, W)

                # # Sample ray
                # x1, y1 = torch.meshgrid(
                #     torch.linspace(-self.sensor_size[0]/2, self.sensor_size[0]/2, self.sensor_res[0]+1, device=self.device)[1:],
                #     torch.linspace(low_i * self.pixel_size, up, self.sensor_res[1]+1, device=self.device)[1:],
                #     indexing='xy'
                # )
                ...
                # pupilz, pupilr = self.exit_pupil()
                # o2 = self.sample_pupil(self.sensor_res, vpp, pupilr=pupilr, pupilz=pupilz)

                # o = torch.broadcast_to(o, o2.shape)
                # d = o2 - o    # broadcast to [vpp, H, W, 3]

                # ray = Ray(o, d,normalized=False, wvln=wvln, device=self.device)

                # Fetch ray
                sub_ray = ray.index_clone((low_i,low_j), (up_i, up_j))

                # Ray tracing
                sub_ray, _, _ = self.trace(sub_ray)

                # Integral
                img_out[:,:,low_i:up_i,low_j:up_j] = self.render_compute_image(img, depth=depth, scale=scale, ray=sub_ray, coherent=True)
    ...
    def psf_narrow_band(self, points, ks=31, spp=GEO_SPP, center=True):
        """ Compute narrow-band PSF. Currently this function is non-differentiable.
        ...
        for i, wvln in enumerate(WAVE_RED):
            psf_r.append(self.psf(points=points, wvln=wvln, ks=ks, spp=spp, center=center))
        psf_r = torch.stack(psf_r, dim = -3).mean(dim=-3)

        psf_g = []
        for i, wvln in enumerate(WAVE_GREEN):
            psf_g.append(self.psf(points=points, wvln=wvln, ks=ks, spp=spp, center=center))
        psf_g = torch.stack(psf_g, dim = -3).mean(dim=-3)

        psf_b = []
        for i, wvln in enumerate(WAVE_BLUE):
            psf_b.append(self.psf(points=points, wvln=wvln, ks=ks, spp=spp, center=center))
    ...
    def psf_board_band(self, points, ks=31, spp=GEO_SPP, recenter=True):
        ...
        # # Manually shift PSF to center (use green PSF center)
        # if recenter:
        #     if point[0] == 0 and point[1] == 0:
        #         psfc_j = ks // 2
        #         psfc_i = ks // 2
        #     else:
        #         psfc_i, psfc_j = torch.unravel_index(torch.argmax(psfs[1, ...]), psfs[1, ...].shape)
        #         psfc_i, psfc_j = psfc_i.item(), psfc_j.item()
            
        #     psfs = nnF.pad(psfs, [ks//2, ks//2, ks//2, ks//2], mode='constant', value=0)
        #     psfs = psfs[:, psfc_i:psfc_i+ks, psfc_j:psfc_j+ks]
    ...
    def pupil_field(self, point, wvln=DEFAULT_WAVE, spp=COHERENT_SPP):
        ...
        # PSF center (on the sensor plane) is determined by chief ray
    ...
    def psf_coherent(self, point, ks=101, wvln=DEFAULT_WAVE, spp=COHERENT_SPP):
        ...
        psfc_idx_i = ((2 - psfc[1]) * h / 4).round().long() # consider both interplation and padding
    ...
    def psf_coherent_old(self, point, ks=101, wvln=DEFAULT_WAVE, spp=COHERENT_SPP, center=True, full_res=False):
        """ Compute complex PSF (both amplitute and phase) by coherent ray tracing and exit-pupil diffraction method. 
        
            To calculate incoherent PSF, please use self.psf() function instead. This function calcualtes coherent PSF, but without pupil diffraction. This function should produce similar results with incoherent PSF. 

            TODO: if point is [3] (point), then output is [ks, ks]. If point is [N, 3], then output is [N, ks, ks]. This is not consistent.

            TODO: check coherent integral, as I have modified the forward_integral function.

        Args:
            point (list, optional): Point source position. Defaults to [0, 0, -10000].
            wvln (float, optional): wvln. Defaults to 0.589.
            spp (int, optional): Sample per pixel. Defaults to 10000000.
            ks (int, optional): Kernel size. Defaults to 31.
            center (bool, optional): Use spot center as PSF center. Defaults to False.

        Returns:
            psf: Complex PSF, shape of [ks, ks] or [H, W].
        """
        assert spp >= 1000000, "spp is too small, may cause inaccurate results."
        ...
        # Ray tracing to sensor plane
        scale = self.calc_scale_ray(point[0, 2].item())
        ...
        # Ideal ray center on the sensor plane by pespective
        pointc_ideal = point.clone()[:,:2]
        pointc_ideal[:, 0] *= self.sensor_size[1] / 2
        pointc_ideal[:, 1] *= self.sensor_size[0] / 2
        ...
        if full_res:
            # Calculate a full-resolution complex field for further computation (e.g., exit-pupil diffraction)
            # Use full sensor resolution as kernel size
            pointc_ref = torch.zeros_like(pointc_ideal)
            psf_full_res = forward_integral(ray, ps=self.pixel_size, ks=self.sensor_res[0], pointc_ref=pointc_ref, coherent=True)
            
            # But the actual PSF center is determined by chief ray
            psf_center_i = int((1 - pointc_chief_ray[:,1] / self.sensor_size[1] * 2) * self.sensor_res[0] / 2)
            psf_center_j = int((1 + pointc_chief_ray[:,0] / self.sensor_size[0] * 2) * self.sensor_res[1] / 2)
            psf_center = [psf_center_i, psf_center_j]

            if single_point:
                psf_full_res = psf_full_res[0, ...]

            return psf_full_res, psf_center
        
        else:
            # Calculate coherent PSF (ks * ks)
            if center:
                psf = forward_integral(ray, ps=self.pixel_size, ks=ks, pointc_ref=pointc_chief_ray, coherent=True)
            else:
                psf = forward_integral(ray, ps=self.pixel_size, ks=ks, pointc_ref=pointc_ideal, coherent=True)

            # # ==> Debug difference with incoherent PSF
            # psf_inten_coherent = psf.abs()**2
            # psf_inten_coherent = psf_inten_coherent / torch.sum(psf_inten_coherent)
            # # psf_inten_coherent = psf_inten_coherent.cpu()
            # psf_inten_incoherent = self.psf(point, ks=ks, center=center, spp=1000000)
            # psf_inten_incoherent = psf_inten_incoherent / torch.sum(psf_inten_incoherent)
            # rel_error = (psf_inten_coherent - psf_inten_incoherent).abs() / (psf_inten_incoherent + EPSILON)
            # abs_error = (psf_inten_coherent - psf_inten_incoherent).abs()

            # Normalize to 1
            # psf_amp = psf.abs().sqrt()
            # scale = torch.sqrt(1 / torch.sum(grid_amp**2))
            # grid_amp = grid_amp * scale
            # grid = grid_amp * torch.exp(1j * grid.angle())
    ...
    # TODO: also implement exit-pupil ray tracing and Rayleigh-Sommerfeld diffraction
    # def psf_RS(self, point, ks=100, wvln=DEFAULT_WAVE, spp=COHERENT_SPP, memory_saving=True):
    #     """ Single point monochromatic PSF of the DoeLens.

    #         Reference: Optical Aberrations Correction in Postprocessing Using Imaging Simulation.

    #         FIXME: this function only supports aperture diffraction, DOE (phase modulator) can not be used!

    #     Steps:
    #     1, Coherent ray tracing to aperture plane.
    #     2, Super position by Rayleigh Sommereld diffraction.
    ...
    #     doe = self.doe
    #     ps = geolens.pixel_size    # Sensor pixel size
    #     k = 2 * math.pi / (ray.wvln * 1e-3)    # wave number
    #     if not torch.is_tensor(point):
    #         point = torch.tensor(point)
    #     if len(point.shape) == 1:
    #         point = point.unsqueeze(0)
    ...
    #     point_obj = point.clone()
    #     point_obj[:, 0] = point[:, 0] * scale * geolens.sensor_size[1] / 2   # x coordinate
    #     point_obj[:, 1] = point[:, 1] * scale * geolens.sensor_size[0] / 2   # y coordinate
    #     psfc = geolens.psf_center(point_obj).squeeze(0)   # shape [2]
    ...
    #         torch.linspace(psfc[0] - ks/2*ps, psfc[0] + ks/2*ps, ks, device=doe.device),
    #         torch.linspace(psfc[1] + ks/2*ps, psfc[1] - ks/2*ps, ks, device=doe.device),
    #         indexing='xy'
    #     )

    #     if memory_saving:
    #         sensor_field = torch.zeros([ks, ks], device=doe.device) + 0j
    #         step_size = 2
    #         for i in range(0, ks, step_size):
    #             for j in range(0, ks, step_size):
    #                 x1 = ray.o[..., 0].unsqueeze(-1).unsqueeze(-1).repeat(1, 1, step_size, step_size)    # shape of [spp, spp, step_size, step_size]
    #                 y1 = ray.o[..., 1].unsqueeze(-1).unsqueeze(-1).repeat(1, 1, step_size, step_size)    # shape of [spp, spp, step_size, step_size]
    #                 opl1 = ray.opl.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, step_size, step_size)    # shape of [spp, spp, step_size, step_size]
    #                 z = self.geolens.d_sensor - doe.d
    #                 x2 = psf_x[i:i+step_size, j:j+step_size]    # shape of [step_size, step_size]
    #                 y2 = psf_y[i:i+step_size, j:j+step_size]

    #                 r2 = (x2 - x1)**2 + (y2 - y1)**2 + z**2
    #                 opl2 = torch.sqrt(r2)
                    
    #                 obliq = (ray.d[..., 2] + z / opl2)

    #                 sensor_field_patch = torch.sum(1 / opl2 * obliq * torch.exp(1j * torch.fmod(k * (opl1 + opl2), 2 * math.pi)), (0, 1))

    #                 # Calculate the complex field at the sensor plane
    #                 sensor_field[i:i+step_size, j:j+step_size] = sensor_field_patch
    #     else:
    #         raise Exception('Not implemented yet.')
    #         sensor_field = RayleighSommerfeldIntegral(wavefront, wavefront_x, wavefront_y, geolens.d_sensor-doe.d, wvln, psf_x, psf_y)

        
    #     psf = sensor_field.abs()**2
    #     psf /= psf.sum()    # shape of [ks, ks]

    #     return psf
    

    # def psf_map_RSD(self, grid=7, ks=100, wvln=0.589, depth=DEPTH):
    #     """ Calculate single wvln PSF map of the doelens.

    #         For each grid point source, we call self.psf() to calculate its PSF. 

    #     Args:
    #         grid (int, optional): grid number. Defaults to 7.
    #         ks (int, optional): size of the PSF kernel. Defaults to 101.
    #         wvln (float, optional): wvln. Defaults to 0.589 um.
    #         depth (float, optional): depth of the point source. Defaults to DEPTH.

    #     Returns:
    #         psf_map (torch.Tensor): PSF map. Shape [H, W].
    #     """
    #     raise Exception('This method is very slow, try to abandon this in the future.')
    #     # 2D point source grid
    #     geolens = self.geolens
    #     points = geolens.point_source_grid(depth=depth, grid=grid, center=True)
    #     points = points.reshape(-1, 3)

    #     # Calculate PSF for each point source
    #     psfs = []
    #     for i in range(points.shape[0]):
    #         point = points[i, ...]
    #         psf = self.psf_RS(point=point, ks=ks, wvln=wvln)
    #         psfs.append(psf)
    ...
    # def psf_map_rgb_RSD(self, grid=7, ks=100, depth=DEPTH):
    #     """ Calculate RGB PSF map of the doelens.
    #     """
    #     psf_maps = []
    #     for wvln in WAVE_RGB:
    #         psf_map = self.psf_map_RSD(grid=grid, ks=ks, wvln=wvln, depth=depth)
    #         psf_maps.append(psf_map)
    #     psf_map = torch.stack(psf_maps, dim=0)   # shape [3, H, W]
    #     return psf_map
    ...
    def foc_plane(self):
        ...
        # focus_p = focus_p[~np.isnan(focus_p) & (focus_p>0)]
        # infocus_sensor_d = float(np.mean(focus_p))
    ...
    def calc_magnification3(self, depth):
        ...
        # # x1 = o1[:, :,:,0].cpu() * mag
        # # y1 = o1[:, :,:,1].cpu() * mag
        # # x2 = torch.mean(o2[:, :,:,0], axis=0).cpu()
        # # y2 = torch.mean(o2[:, :,:,1], axis=0).cpu()
    ...
    def prune_surf(self, outer=None, surface_range=None):
        ...
        if self.is_cellphone:
            ...
            # ==> 3. Front surface should be smaller than back surface. But this does not apply to fisheye lens.
            # for i in surface_range[:-1]:
            #     if self.surfaces[i].mat1.A < self.surfaces[i].mat2.A:
            ...
            # outer = 0.1 if outer is None else outer # RMS lens design
    ...
    def draw_spot_diagram(self, M=7, depth=DEPTH, wvln=DEFAULT_WAVE, save_name=None):
        ...
        #         # ra_ = ra[:,M-1-i,M-1-j]
        #         ra_ = ra[:,i,j]
        #         x, y = o2[:,i,j,:2][:,0], o2[:,i,j,:2][:,1]
        #         x, y = x[ra_>0], y[ra_>0]
        #         xc, yc = x.sum()/ra_.sum(), y.sum()/ra_.sum()
        #         ax.scatter(x, y, 1, 'b', 'o')
        #         ax.scatter([xc], [yc], None, 'r', 'x')
        #         # ax.axis('scaled')
        #         # # axs[i, j].set(aspect='equal')
        #         # ax.set_aspect('equal')
        #         # axs[i, j].set_xlabel('xlabel', fontsize=2)
        #         # axs[i, j].set_ylabel('ylabel', fontsize=2)
    ...
    @torch.no_grad()
    def rms_map(self, res=(32, 32), depth=DEPTH):
        ...
        Returns:
            rms_map (torch.Tensor): RMS map as a weight mask.
        """
        M = 64
        scale = - depth * np.tan(self.hfov) / self.r_last
    ...
    def analysis_rms(self, depth=DEPTH):
        ...
        # Point center determined by green light TODO: can use pointc func
    ...
    def loss_rms(self, depth=DEPTH):
        ...
        for wvln in WAVE_RGB:
            ...
            # # to visualize batch psf
            # # o2_norm shape is [spp, H, W, 2]
            # tmp=o2_norm[:,0,0,:]
            # plt.scatter(x=tmp[:,0].cpu().numpy(), y=tmp[:,1].cpu().numpy())

            # rms += ((o2_norm**2).sum(dim=-1).sum(0)/ray.ra.sum(0).add(0.0001)).sqrt() # rms per pixel, first compute r^2 error, then average spp
    ...
    def loss_self_intersec(self, dist_bound=0.2, thickness_bound=0.4, flange_bound=0.6):
        ...
        # Calculate distance to the sensor (2 * dist_bound)
    ...
    def get_optimizer_params(self, lr=[1e-4, 1e-4, 1e-1, 1e-4], decay=0.01, diff_surf_range=None):
        ...
        for i in diff_surf_range:
            ...
            # elif isinstance(surf, AspCubic):
            #     raise Exception('AspCubic not supported yet.')
    ...
    def optimize(self, lrs=[5e-4, 1e-4, 0.1, 1e-2], decay=0.02, iterations=2000, test_per_iter=100, centroid=True, importance_sampling=True, dropout=False,  result_dir=None):
        """ Optimize the lens by minimizing rms errors.

        Debug hints:
            1, thickness (fov and ttl should match)
            2, loss_reg
            3, alpha order (higher is btter but more sensitive)a
            4, dropout ratio (in fine tuning, prefer no dropout)
            5, learning rate and decay (prefer smaller lr and decay)
        ...
        if result_dir is None:
            result_dir = './results/' + datetime.now().strftime("%m%d-%H%M%S") + '-DesignLens'
        else:
        ...
        for i in range(iterations+1):
            ...
            for j, wv in enumerate(WAVE_RGB):
                ...
                # Weight
                # weight_mask = xy_norm.detach().abs().sum([0, -1])   # Use L1 error as weight mask
                ...
                # # => Baseline
                # l_rms = torch.sum(xy_norm.abs().sum(-1)) / (torch.sum(ray.ra) + EPSILON)
                # loss_rms.append(l_rms)
    ...
    # def curriculum_design(self, lrs=[5e-4, 1e-4, 0.1, 1e-2], decay=0.01, iterations=2000, result_dir='./results'):
    #     """ Design a lens by curriculum learning startegy.

    #     Experience:
    #         1, for aspherical lens design, use [5e-4, 1e-4, 0.1, 1e-4]
    #         2, for spherical lens design, use [1e-4, 1e-4, 0, 0]
    #     """
    ...
    #     diag_target = self.r_last * 2
    #     fnum_target = self.fnum
    #     step_ls = np.linspace(0, curriculum_steps, curriculum_steps)
    #     diag_ls = diag_target * (curriculum_start + (1 - curriculum_start) * np.sin(step_ls / curriculum_steps * math.pi/2))
    #     fnum_ls = fnum_target / (curriculum_start + (1 - curriculum_start) * np.sin(step_ls / curriculum_steps * math.pi/2))

    #     for step in range(curriculum_steps):
    #         # ==> Design target for this step
    #         self.r_last = diag_ls[step] / 2
    #         self.hfov = np.arctan(self.r_last / self.foclen)

    #         self.fnum = fnum_ls[step]
    #         aper_r = self.foclen / self.fnum / 2
    #         self.surfaces[self.aper_idx].r = aper_r

    #         # ==> Save starting point
    #         self.analysis(save_name=f'{result_dir}/step{step}_starting_point', zmx_format=True)
    #         self.write_lens_json(f'{result_dir}/step{step}_starting_point.json')
    #         print(f'Curriculum learning step {step}, target: FOV {round(self.hfov * 2 * 57.3, 2)}, DIAG {round(2 * self.r_last, 2)}mm, F/{self.fnum}.')
            
    #         # ==> Lens design using RMS errors
    #         iterations = curriculum_iters * (step + 1)
    #         self.refine(lrs=lrs, decay=decay, iterations=iterations, test_per_iter=test_per_iter, importance_sampling=False, result_dir=result_dir, shape_control=True)

    #     # ==> Refine lens at the last step
    #     self.refine(iterations=1000, test_per_iter=100, centroid=False, importance_sampling=True, result_dir=result_dir)
    #     self.refine(iterations=1000, test_per_iter=100, centroid=True, importance_sampling=True, shape_control=False, result_dir=result_dir)

    #     self.analysis(save_name=f'{result_dir}/final_design', zmx_format=True)
    #     self.write_lens_json(f'{result_dir}/final_design.json')
    ...
    def read_lensfile(self, filename, use_roc=False):
        """ read lens info from structured .txt file.
            ...
            TODO: current lens file is too urgly, we should use json file in the future.
        ...
        with open(filename) as file:
            ...
            for line in file:
                if line_no < 2: # first two lines are comments; ignore them
                    ...
                    # roc = 1 / float(ls[2]) if float(ls[2]) != 0 else 0  # roc: radius of curvature
                    
                    mat_name = 'air' if len(ls) <= 4 else ls[4]
                    materials.append(Material(mat_name))
                    ...
                    # Cubic face
                        ...
                        if len(ls) <= 5:
                            surfaces.append(Cubic(r, d_total))
                        elif len(ls) > 5:
                            ai = []
                            for ac in range(5, len(ls)):
                                ai.append(float(ls[ac]))
                            surfaces.append(Cubic(r, d_total, ai))
                        else:
                            raise NotImplementedError()
                    
                    # DOE
                    elif surface_type == "DOE":
                        surfaces.append(DOE_GEO(l = r, d = d_total))    # in the lens file, we store side length

                    # # Freeform lens
                    # elif surface_type == "F":
                    #     if len(ls) <= 5:
                    #         surfaces.append(Freeform(r, d_total))
                    ...
                    # # Odd Aspheric. Polynomial surface.
                    # elif surface_type == 'P': 
                    #     if use_roc:
                    #         c = 1/roc if roc!=0 else 0
                    #     else:
                    ...
                    #     # spheric surface
                    #     if len(ls) <= 5:
                    #         surfaces.append(AsphericOdd(r, d_total, c, device=self.device))
                    #     # aspheric surface with only conic term
                    #     elif len(ls) == 6:
                    #         conic = float(ls[5])
                    #         ai = None
                    #         surfaces.append(AsphericOdd(r, d_total, c, conic, ai, device=self.device))
                    #     # aspheric surface
                    #     else:
                    #         ai = []
                    #         for ac in range(5, len(ls)):
                    ...
                    # Spheric or aspheric surface TODO: rewrite this part using queue structure
                    elif surface_type == 'S': 
                        if use_roc:
                            assert roc > 1 or roc < -1 or roc == 0, 'radius-of-curvature is used, but the data looks like curvature.'
                            c = 1/roc if roc!=0 else 0
                        else:
                            c = roc
    ...
    def write_lensfile(self, filename='./test.txt', str1='optimized lens file.\n', write_zmx=False):
        ...
        """ 
        raise Exception('This function has been deprecated. Use write_lens_json instead.')
        f = open(filename, 'w')
        f.writelines(str1)
        f.writelines('type    distance   roc      diameter    material\n')
        f.writelines('O   0         0         0        AIR\n')
        for i in range(len(self.surfaces)):
            # AsphericOdd surface, it is son class, so should be placed first
            if isinstance(self.surfaces[i], AsphericOdd):
                if i == 0:
                    str2 = f'P {self.surfaces[i].d.item():.3f} '
                else:
                    str2 = f'P {self.surfaces[i].d.item() - self.surfaces[i-1].d.item():.3f} '
                
                str2 = str2 + f'{self.surfaces[i].c.item():.8f} '
                ...
                if self.surfaces[i].ai is not None:
                    if self.surfaces[i].ai_degree == 4:
                        str2 = str2 + f'{self.surfaces[i].ai4.item():e} {self.surfaces[i].ai6.item():e} {self.surfaces[i].ai8.item():e}'
                    elif self.surfaces[i].ai_degree == 5:
                        str2 = str2 + f'{self.surfaces[i].ai4.item():e} {self.surfaces[i].ai6.item():e} {self.surfaces[i].ai8.item():e} {self.surfaces[i].ai10.item():e}'
                    else:
            ...
            # Aspheric surface
            elif isinstance(self.surfaces[i], Aspheric):
                if i == 0:
                    str2 = f'S {self.surfaces[i].d.item():.4f} '
                else:
                    str2 = f'S {self.surfaces[i].d.item() - self.surfaces[i-1].d.item():.4f} '
            ...
            # Cubic phase surface
            elif isinstance(self.surfaces[i], Cubic):
                if i == 0:
                    str2 = f'C {self.surfaces[i].d.item():.3f} '
                else:
                    str2 = f'C {self.surfaces[i].d.item() - self.surfaces[i-1].d.item():.3f} '
                
                str2 = str2 + f'{0.00} '    # original c, but for cubic phase, c = 0
                str2 = str2 + f'{self.surfaces[i].r*2:.2f} '
                str2 = str2 + f'{self.materials[i+1].name} '

                if self.surfaces[i].b_degree >= 3:
                    str2 = str2 + f'{self.surfaces[i].b3.item():.10f} '
                if self.surfaces[i].b_degree >= 5:
                    str2 = str2 + f'{self.surfaces[i].b5.item():.10f} '
                if self.surfaces[i].b_degree >= 7:
                    str2 = str2 + f'{self.surfaces[i].b7.item():.10f} '
                
                f.writelines(str2+'\n')

            # Hybrid surface
            elif isinstance(self.surfaces[i], AspCubic):
                if i == 0:
                    str2 = f'S {self.surfaces[i].d.item():.4f} '
                else:
                ...
                for j in range(1, self.surfaces[i].ai_degree+1):
                    ai = eval(f'self.surfaces[i].ai{2*j}.item()')
                    str2 = str2 + f"{ai:.8f} "

                if self.surfaces[i].b_degree >= 3:
                    str2 = str2 + f'{self.surfaces[i].b3.item():.8f} '
                if self.surfaces[i].b_degree >= 5:
                    str2 = str2 + f'{self.surfaces[i].b5.item():.8f} '
                if self.surfaces[i].b_degree >= 7:
                    str2 = str2 + f'{self.surfaces[i].b7.item():.8f} '
    ...
    def write_lens_json(self, filename='./test.json'):
        ...
        for i, s in enumerate(self.surfaces):
            ...
            #     # this will be deprecated after updating all lens files
        ...
        with open(filename, 'r') as f:
            ...
            for surf_dict in data['surfaces']:
                ...
                if surf_dict['type'] == 'Aperture':
                    ...
                    # s = AspheCubic(c=1/surf_dict['roc'], r=surf_dict['r'], d=d, k=surf_dict['k'], ai=surf_dict['ai'], b=surf_dict['b'], mat1=surf_dict['mat1'], mat2=surf_dict['mat2'])
...
#     def write_zmx_old(self, filename='./test.zmx'):
#         """ Write the lens into .zmx file.

#             now it only supports highly conditioned lens files.
#         """
...
#             # Aspheric surface
#             # ================================
#             elif isinstance(s, Aspheric):
#                 # Next surface is glass
#                 if s.mat2.glassname != 'air':
#                     surf_str = f"""SURF {i+1}
#     TYPE EVENASPH
#     FIMP 
...
#                     surf_str = surf_str + f"""
#     DISZ {self.surfaces[i+1].d.item() - self.surfaces[i].d.item():.6e}
#     GLAS {s.mat2.glassname} 0 0 {s.mat2.n} {s.mat2.V}
#     CONI {s.k.item():.6e}
#     VDSZ 0 0
#     DIAM {s.r} 1 0 0 1 ""
