...
def plane_wave_field(phy_size, res, wvln=0.589, z=0.):
...
def point_source_field(point=[0, 0, -1000.], phy_size=[2, 2], res=[1024, 1024], wvln=0.589, fieldz=0):
    ...
    ps = phy_size[0] / res[0]
    x, y = torch.meshgrid(
        torch.linspace(-0.5 * phy_size[0] + 0.5 * ps, 0.5 * phy_size[1] - 0.5 * ps, res[0]),
        torch.linspace(0.5 * phy_size[1] - 0.5 * ps, -0.5 * phy_size[0] + 0.5 * ps, res[1]), 
        indexing='xy')
    z = torch.full_like(x, fieldz)
    
    if point[2] < -100. and torch.get_default_dtype() != torch.float64:
        r = (z - point[2]) + ((x - point[0])**2 + (y - point[1])**2) / (z - point[2])
        phi = torch.remainder(k * ((x - point[0])**2 + (y - point[1])**2) / (z - point[2]), 2 * np.pi)
    else:
...
def batch2field(img, phy_size, z=0, wvln=0.589, padding=False, phase='zero', device='cuda'):
    ...
    res = u.shape
    field = ComplexWave(u=u, phy_size=phy_size, valid_phy_size=valid_phy_size, res=res, wvln=wvln, device=device)
    field.z += z
