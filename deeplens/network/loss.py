...
class PSFAlignLoss(nn.Module):
    ...
    def calc_center(self, psf):
        ...
        Input:
            PSF: shape [3, ks, ks] or [N, 3, ks, ks]
        """
        device = psf.device
        if len(psf.shape) == 3:
            center_x = torch.sum(psf * self.x.to(device), dim=(1,2)) / torch.sum(psf, dim=(1,2))
            center_y = torch.sum(psf * self.y.to(device), dim=(1,2)) / torch.sum(psf, dim=(1,2))
            return torch.stack([center_x, center_y], dim=-1) # shape of [3, 2]
        elif len(psf.shape) == 4:
            center_x = torch.sum(psf * self.x.to(device), dim=(2,3)) / torch.sum(psf, dim=(2,3))
            center_y = torch.sum(psf * self.y.to(device), dim=(2,3)) / torch.sum(psf, dim=(2,3))
            return torch.stack([center_x, center_y], dim=-1) # shape of [N, 3, 2]
    ...
    def forward(self, psf):
        ...
        """
        if len(psf.shape) == 3:
            centroid_psf = self.calc_center(psf)
            loss = torch.mean((centroid_psf[0, :] - centroid_psf[1, :]) ** 2 + (centroid_psf[1, :] - centroid_psf[2, :]) ** 2 + (centroid_psf[2, :] - centroid_psf[0, :]) ** 2)
        elif len(psf.shape) == 4:
            centroid_psf = self.calc_center(psf)
            loss = torch.mean((centroid_psf[:, 0, :] - centroid_psf[:, 1, :]) ** 2 + (centroid_psf[:, 1, :] - centroid_psf[:, 2, :]) ** 2 + (centroid_psf[:, 2, :] - centroid_psf[:, 0, :]) ** 2)
